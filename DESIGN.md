...
- **i18n** *(avançada des de la Fase 4)*: català, castellà, anglès. **A ✅** (infra gen-l10n + contingut de preparació, `3619470`) · **B ✅** (UI del flux de joc + 14 noms oficials de cabanes, `7ed6b1c`) · **C pendent**: catàleg de tiles i noms d'expansions/mòduls per id estable via ARB (Xocolatl!), cos de la Home, pantalles de score/regles. Terminologia canònica per idioma: `docs/spec-fase-i18n.md` §5.
...
- **Catàleg estàtic: JSON+seed → constants Dart (decidir a la fase i18n)** — AJORNAT: el catàleg (boardgames/modules/tiles) és 100% estàtic però viu duplicat (JSON de seed + BD drift). Quan la i18n mogui noms i descripcions a l'ARB (estratègia: un únic ARB per idioma, catàleg inclòs via claus derivades de l'id estable, p. ex. `tileName_base_jungle_water`), el JSON quedarà reduït a dades d'identitat: valorar llavors convertir-lo en constants Dart darrere de `BoardgameRepository` i retirar el seeding. La inversió de Clean Architecture (feta 2026-07-18) fa el canvi contingut: només es toca la implementació del repositori. Drift es manté igualment per a la dada dinàmica de la Fase 2 (historial, perfils). Les descripcions buides de `modules.json` NO s'han d'omplir: aniran a l'ARB.
